...
export default function MatchsPage({ profile }: MatchsPageProps) {
  ...
  function getPredBadge(match: MatchWithPred) {
    ...
    if (pts >= 7) return { label: '✓ Score exact', color: 'bg-green-100 text-green-700 border border-green-200', pts }
    if (pts >= 3) return { label: '✓ Bon résultat', color: 'bg-blue-100 text-blue-700 border border-blue-200', pts }
    return { label: '✗ Raté', color: 'bg-red-100 text-red-700 border border-red-200', pts: 0 }
  }
      ...
      {finishedMatches.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
            <CheckCircle className="w-4 h-4 text-green-500" /> Résultats
          </h2>
                ...
                <div key={match.id} className="card card-hover rounded-xl p-4 cursor-pointer" onClick={() => setSelectedMatch(match)}>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <span className={`text-sm font-semibold ${actualWinner === 'home' ? 'text-dark' : 'text-gray-400'}`}>{match.team_home}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-lg font-black text-dark">{match.score_home}</span>
                      <span className="text-gray-300 font-bold">–</span>
                      <span className="text-lg font-black text-dark">{match.score_away}</span>
                    </div>
                    <div className="flex-1 flex items-center gap-2 justify-end">
                      <span className={`text-sm font-semibold ${actualWinner === 'away' ? 'text-dark' : 'text-gray-400'}`}>{match.team_away}</span>
                  ...
                  {match.prediction && (
                    <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                      <div className="text-xs text-gray-400">
                        Mon prono : <span className="font-semibold text-gray-600">{match.prediction.predicted_home} – {match.prediction.predicted_away}</span>
                      </div>
                  ...
                  {!match.prediction && (
                    <div className="mt-3 pt-3 border-t border-gray-100">
                      <span className="text-xs text-gray-400 italic">Pas de pronostic</span>
                    </div>
                  )}
                </div>
              )
              ...
              return (
                <div key={match.id} className="card card-hover rounded-xl p-4 cursor-pointer" onClick={() => setSelectedMatch(match)}>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 flex items-center gap-2">
                      <span className="text-xl">{match.flag_home}</span>
                      <span className="text-sm font-semibold text-dark">{match.team_home}</span>
                    </div>
                    <div className="text-xs text-gray-400 font-medium">
                      {format(new Date(match.kickoff_at), 'EEE d MMM · HH:mm', { locale: fr })}
                    </div>
                    <div className="flex-1 flex items-center gap-2 justify-end">
                      <span className="text-sm font-semibold text-dark">{match.team_away}</span>
                      <span className="text-xl">{match.flag_away}</span>
                    </div>
                  </div>
